...
- [`android/`](android/) — overlay `12.10.1-a-gram.18` для официального Telegram Android.
...
- в шапке диалогов показываются режим маршрута, IP и приблизительное гео текущей Telegram-сессии; отдельный экран управляет запуском Tor, новой цепочкой контейнера, общим перезапуском и зашифрованными obfs4/webtunnel-мостами;
- запуск Tor контролируется одним фоновым bootstrap-процессом с прогрессом, таймаутом и чистым повторным запуском; при ошибке соединение остаётся fail-closed;
