...
win:
    python -c "p='build_libvpx_win.sh';s=open(p,encoding='utf-8').read();n=chr(10);old='make -j'+chr(36)+'NUMBER_OF_PROCESSORS'+n+'make -j'+chr(36)+'NUMBER_OF_PROCESSORS install';new='make -j1 target=libs'+n+'make -j1 target=solution Release_x64'+n+'make -j1 NO_LAUNCH_DEVENV=1 install';open(p,'w',encoding='utf-8',newline='').write(s.replace(old,new))"
mac:
...
stage('jom', """
win:
    curl.exe -L --fail --output ./jom.zip https://ftp.fau.de/qtproject/official_releases/jom/jom_1_1_3.zip
    python -c "import hashlib,sys;sys.exit(hashlib.sha256(open('jom.zip','rb').read()).hexdigest()!='128fdd846fe24f8594eed37d1d8929a0ea78df563537c0c1b1861a635013fff8')"
    mkdir jom
    tar.exe -xf ./jom.zip -C ./jom
    del jom.zip
""", 'ThirdParty')
