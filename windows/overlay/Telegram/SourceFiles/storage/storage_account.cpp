...
void Account::writePrefsNow() {
	writePrefs();
}

bool Account::writeSecretMedia(
		const QString &name,
		const QByteArray &bytes) {
	if (name.isEmpty()
		|| name.size() > 96
		|| std::any_of(name.begin(), name.end(), [](QChar ch) {
			return !ch.isLetterOrNumber() && ch != '-' && ch != '_';
		})) {
		return false;
	}
	auto data = EncryptedDescriptor(bytes.size() + sizeof(quint32));
	data.stream << bytes;
	if (data.stream.status() != QDataStream::Ok) {
		return false;
	}
	auto file = FileWriteDescriptor(u"agram_"_q + name, _basePath, true);
	file.writeEncrypted(data, _localKey);
	return true;
}

QByteArray Account::readSecretMedia(const QString &name) const {
	if (name.isEmpty()
		|| name.size() > 96
		|| std::any_of(name.begin(), name.end(), [](QChar ch) {
			return !ch.isLetterOrNumber() && ch != '-' && ch != '_';
	...
	if (!ReadEncryptedFile(file, u"agram_"_q + name, _basePath, _localKey)) {
		return {};
	}
	auto result = QByteArray();
	file.stream >> result;
	return (file.stream.status() == QDataStream::Ok) ? result : QByteArray();
}
