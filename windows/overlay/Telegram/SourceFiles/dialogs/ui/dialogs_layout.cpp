...
void PaintRow(
	...
	if (const auto folder = entry->asFolder()) {
		...
		const auto customPreview = secret
			? secret->chatListCustomPreview()
			: QString();
		if (!customPreview.isEmpty() && date) {
			PaintDialogDate(p, entry, fakeRow, date, rectForName, context);
		}
		auto availableWidth = namewidth;
		if (!customPreview.isEmpty()) {
			availableWidth = PaintWideCounter(
				p,
				context,
				badgesState,
				texttop,
		...
		if (!customPreview.isEmpty()) {
			...
			p.drawTextLeft(
				...
				st::dialogsTextFont->elided(
					customPreview,
					availableWidth));
		} else if (!ShowSendActionInDialogs(thread)
			|| !thread->sendActionPainter()->paint(
				p,
				nameleft,
				texttop,
	...
	if (flags
		...
		const auto &lock = ThreeStateIcon(
			st::dialogsSecretLockIcon,
			context.active,
			context.selected);
		const auto lockTop = rectForName.top()
			+ (st::semiboldFont->height - lock.height()) / 2;
		lock.paint(p, rectForName.left(), lockTop, context.width);
		rectForName.setLeft(
			rectForName.left() + lock.width() + st::dialogsChatTypeSkip);
		p.setPen(context.active
			? st::dialogsNameFgActive
			: context.selected
			? st::dialogsSecretNameFgOver
			: st::dialogsSecretNameFg);
		rowName.draw(p, {
			.position = rectForName.topLeft(),
			.availableWidth = rectForName.width(),
			.elisionLines = 1,
...
void RowPainter::Paint(
	...
	const auto displayDate = [&]() -> TimeId {
		...
		const auto customDate = (!item && !cloudDraft)
			? entry->chatListTimeId()
			: TimeId(0);
		return std::max({ itemDate, draftDate, customDate });
	}();
