...
void ResolveSettings(not_null<Main::Session*> session, Settings &settings) {
	...
	if (!settings.onlySinglePeer()) {
		...
		settings.singlePeerSearchFrom = MTP_inputPeerEmpty();
		settings.singlePeerSearchFromId = 0;
		settings.singlePeerSearchFromName.clear();
		settings.singlePeerSearchFromUsername.clear();
		if (settings.format == Output::Format::HtmlAndPdf) {
			settings.format = Output::Format::Html;
		}
	}
}
...
void PanelController::fillParams(const PasswordCheckState &state) {
	...
	_settings->singlePeerSearchFrom = state.singlePeerSearchFrom;
	_settings->singlePeerSearchFromId = state.singlePeerSearchFromId;
	_settings->singlePeerSearchFromName = state.singlePeerSearchFromName;
	_settings->singlePeerSearchFromUsername = state.singlePeerSearchFromUsername;
	if (_settings->onlySinglePeerFromUser()) {
		_settings->format = Output::Format::HtmlAndPdf;
