...
std::unique_ptr<AbstractWriter> CreateWriter(Format format) {
	...
	case Format::HtmlAndPdf: return std::make_unique<HtmlAndPdfWriter>();
	}
...
Stats AbstractWriter::produceTestExample(
	...
	const auto singleDialogExample = (settings.format == Format::HtmlAndPdf);
	if (singleDialogExample) {
	...
	if (singleDialogExample) {
		dialogBot.relativePath.clear();
		dialogs.chats.resize(1);
	}
