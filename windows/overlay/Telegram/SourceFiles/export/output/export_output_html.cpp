...
public:
	...
	[[nodiscard]] QByteArray pushHeader(
		...
		const QByteArray &details = QByteArray());
...
QByteArray HtmlWriter::Wrap::pushHeader(
	...
	result.append(popTag());
	if (!details.isEmpty()) {
		result.append(pushDiv("agram_header_details details"));
		result.append(SerializeString(details));
		result.append(popTag());
	}
	result.append(popTag());
	result.append(popTag());
...
Result HtmlWriter::writeDialogOpening(int index) {
	...
	auto filterDetails = QByteArray();
	if (_settings.onlySinglePeerFromUser()) {
		auto who = _settings.singlePeerSearchFromName;
		if (!_settings.singlePeerSearchFromUsername.isEmpty()) {
			who += u" (@"_q
				+ _settings.singlePeerSearchFromUsername
				+ ')';
		}
		const auto bareId = _settings.singlePeerSearchFromId.value
			& PeerId::kChatTypeMask;
		filterDetails = (u"Сообщения пользователя "_q
			+ who
			+ u" · Telegram ID "_q
			+ QString::number(bareId)).toUtf8();
	}
	auto block = _chat->pushHeader(
		name,
		_settings.onlySinglePeer() ? QString() : _dialogsRelativePath,
		filterDetails);
	block.append(_chat->pushDiv(_settings.onlySinglePeerFromUser()
		? "page_body chat_page agram_filtered"
		: "page_body chat_page"));
