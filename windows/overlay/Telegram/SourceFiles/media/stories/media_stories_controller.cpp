...
void Controller::toggleLiked() {
	confirmReaction(crl::guard(this, [=] {
		_reactions->toggleLiked();
	}));
}

void Controller::confirmReaction(Fn<void()> confirmed) const {
	if (!_session || !_session->settings().ghostModeEnabled()) {
		confirmed();
		return;
	}
	uiShow()->show(Ui::MakeConfirmBox({
		.text = u"Реакция будет отправлена собеседнику и раскроет вашу активность."_q,
		...
		for (const auto &suggestedReaction : _suggestedReactions) {
			...
			_areas.push_back({
				...
				.handler = std::make_shared<LambdaClickHandler>([=] {
					...
					if (_session
						&& _session->settings().ghostModeEnabled()) {
						confirmReaction(crl::guard(this, send));
					} else {
						send();
					}
				}),
