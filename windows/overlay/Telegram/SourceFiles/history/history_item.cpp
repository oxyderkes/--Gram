...
void HistoryItem::markAgramDeletedOnServer() {
	if (_agramDeletedOnServer) {
		return;
	}
	_agramDeletedOnServer = 1;
	_history->owner().requestItemViewRefresh(this);
}

void HistoryItem::markAgramSecretMediaLocal() {
	if (_agramSecretMediaLocal) {
		return;
	}
	_agramSecretMediaLocal = 1;
	_history->owner().requestItemViewRefresh(this);
}
