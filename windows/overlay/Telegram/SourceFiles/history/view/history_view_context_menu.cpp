...
bool AddDeleteSelectedAction(
	...
	const auto secretChatId = first
		? first->history()->agramSecretChatId()
		: 0;
	if (secretChatId) {
		menu->addAction(tr::lng_context_delete_selected(tr::now), [=] {
			auto randomIds = std::vector<uint64>();
			const auto entry = request.navigation->session(
			).secretChats().entry(secretChatId);
			if (!entry) {
				return;
			}
			for (const auto &selected : request.selectedItems) {
				if (const auto randomId = entry->randomIdForMessage(
						selected.msgId.msg)) {
			...
			controller->show(Ui::MakeConfirmBox({
				...
				.confirmed = crl::guard(list, [=] {
					request.navigation->session().secretChats().deleteMessages(
						secretChatId,
						randomIds);
					list->cancelSelection();
				}),
...
bool AddDeleteMessageAction(
	...
	if (const auto secretChatId = item->history()->agramSecretChatId()) {
		const auto itemId = item->fullId();
		const auto controller = list->controller();
		menu->addAction(tr::lng_context_delete_msg(tr::now), [=] {
			const auto entry = controller->session(
			).secretChats().entry(secretChatId);
			const auto randomId = entry
				? entry->randomIdForMessage(itemId.msg)
				: 0;
			if (!randomId) {
				return;
			}
			controller->show(Ui::MakeConfirmBox({
				.text = u"Удалить сообщение у обоих участников секретного чата?"_q,
				.confirmed = [=] {
					controller->session().secretChats().deleteMessages(
						secretChatId,
						{ randomId });
				},
				.confirmText = tr::lng_box_delete(),
...
void AddWhenEditedForwardedAuthorActionHelper(
	...
	if (item->history()->owner().hasAgramEditHistory(item->fullId())) {
		if (insertSeparator && !menu->empty()) {
			menu->addSeparator(&st::expandedMenuSeparator);
		}
		menu->addAction(
			tr::lng_agram_edit_history_action(tr::now),
			[=] { ShowAgramEditHistory(controller, item); },
			&st::menuIconEdit);
