...
, _cornerButtons(
	...
	if (!_history->agramSecretChatId()) {
		session().data().restoreAgramDeletedMessages(_peer->id);
		_peer->updateFull();
	}
	...
	_composeControls->sendActionUpdates(
		...
		if (_history->agramSecretChatId() || mode() == Mode::Sublist) {
			return;
		} else if (!data.cancel) {
	...
	if (mode() == Mode::History && !_history->agramSecretChatId()) {
...
void ChatWidget::uploadFile(
		...
		SendMediaType type) {
	if (const auto chatId = _history->agramSecretChatId()) {
		session().secretChats().sendFileBytes(
			chatId,
			fileContent,
			u"file"_q,
			QString());
...
void ChatWidget::sendVoice(const ComposeControls::VoiceToSend &data) {
	if (const auto chatId = _history->agramSecretChatId()) {
		session().secretChats().sendFileBytes(
			chatId,
			data.bytes,
			data.video ? u"video-message.mp4"_q : u"voice-message.ogg"_q,
			data.video ? u"video/mp4"_q : u"audio/ogg"_q);
		_composeControls->cancelReplyMessage();
...
void ChatWidget::sendTextWithTags(
	...
	if (const auto chatId = _history->agramSecretChatId()) {
		...
		if (!text.isEmpty()) {
			session().secretChats().sendText(chatId, text);
		}
		_composeControls->clear();
		finishSending();
		if (done) {
...
bool ChatWidget::sendExistingDocument(
	...
	if (const auto chatId = _history->agramSecretChatId()) {
		...
		const auto name = document->filename().isEmpty()
			? u"sticker.webp"_q
			: document->filename();
		if (!bytes.isEmpty()) {
			session().secretChats().sendFileBytes(
			...
			session().secretChats().sendFile(chatId, path);
		} else {
			controller()->showToast(
				u"Сначала дождитесь загрузки файла на компьютер."_q);
			return false;
		}
...
bool ChatWidget::sendExistingPhoto(
	...
	if (const auto chatId = _history->agramSecretChatId()) {
		...
		auto bytes = media->imageBytes(Data::PhotoSize::Large);
		if (bytes.isEmpty()) {
			bytes = media->imageBytes(Data::PhotoSize::Thumbnail);
		}
		if (bytes.isEmpty()) {
			controller()->showToast(
				u"Сначала дождитесь загрузки фотографии на компьютер."_q);
			return false;
		}
		session().secretChats().sendFileBytes(
			chatId,
...
void ChatWidget::sendInlineResult(
	...
	if (_history->agramSecretChatId()) {
		controller()->showToast(
			u"Inline-боты недоступны в секретных чатах."_q);
		return;
	}
	if (!_canSendMessages) {
		return;
...
Dialogs::RowDescriptor ChatWidget::activeChat() const {
	...
	} else if (const auto secret = secretChatEntry()) {
		return { secret, messageId };
	}
	return { _history, messageId };
}

SecretChats::Entry *ChatWidget::secretChatEntry() const {
	const auto chatId = _history->agramSecretChatId();
	return chatId ? session().secretChats().entry(chatId) : nullptr;
}
...
void ChatWidget::requestSponsoredMessages() {
	if (_history->agramSecretChatId()) {
		return;
	} else if (mode() != Mode::History || _topic) {
		return;
...
rpl::producer<Data::MessagesSlice> ChatWidget::historySource(
	...
	if (_history->agramSecretChatId()) {
		return rpl::single(rpl::empty) | rpl::then(
			session().secretChats().changes()
		) | rpl::map([=] {
			auto items = std::vector<not_null<HistoryItem*>>();
			for (const auto &item : _history->clientSideMessages()) {
				items.push_back(item);
			}
			ranges::sort(items, ranges::less{}, [](not_null<HistoryItem*> item) {
			...
			result.fullCount = int(result.ids.size());
			result.nearestToAround = ranges::contains(
				result.ids,
				aroundId.fullId)
				? aroundId.fullId
				: result.ids.empty()
...
void ChatWidget::listMarkReadTill(not_null<HistoryItem*> item) {
	if (const auto chatId = _history->agramSecretChatId()) {
		session().secretChats().markRead(chatId, item->date());
	} else if (_replies) {
...
void ChatWidget::confirmDeleteSelected() {
	if (const auto chatId = _history->agramSecretChatId()) {
		const auto entry = secretChatEntry();
		if (!entry) {
			return;
		}
		auto randomIds = std::vector<uint64>();
		for (const auto &id : _inner->getSelectedIds()) {
			if (const auto randomId = entry->randomIdForMessage(id.msg)) {
		...
		controller()->show(Ui::MakeConfirmBox({
			.text = u"Удалить выбранные сообщения у обоих участников секретного чата?"_q,
			.confirmed = crl::guard(this, [=] {
				session().secretChats().deleteMessages(chatId, randomIds);
