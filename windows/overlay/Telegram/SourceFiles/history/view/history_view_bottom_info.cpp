...
bool BottomInfo::isWide() const {
	return (_data.flags & Data::Flag::Edited)
		...
		|| (_data.flags & Data::Flag::LocalMedia)
...
void BottomInfo::layoutDateText() {
	const auto deleted = (_data.flags & Data::Flag::Deleted)
		? u"{DELETED} "_q
		: QString();
	const auto localMedia = (_data.flags & Data::Flag::LocalMedia)
		? u"{НА ПК} "_q
		: QString();
	...
	const auto date = deleted + localMedia + (editedPrimary
...
BottomInfo::Data BottomInfoDataFromMessage(not_null<Message*> message) {
	...
	if (item->agramDeletedOnServer()) {
		result.flags |= Flag::Deleted;
	}
	if (item->agramSecretMediaLocal()) {
		result.flags |= Flag::LocalMedia;
	}
