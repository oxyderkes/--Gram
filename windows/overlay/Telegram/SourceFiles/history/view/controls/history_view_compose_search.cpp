...
, _calendarChat(history) {
	...
	rpl::merge(
		...
		const auto local = _history->agramSecretChatId() != 0;
		const auto &apiData = _apiSearch.messages();
		const auto &messages = local ? _localMessages : apiData.messages;
		const auto size = int(messages.size());
		if (!local && index >= (size - 1) && size != apiData.total) {
		...
		if (item) {
			...
			_activations.fire_copy({
				item,
				local ? _localQuery : _apiSearch.request().query,
			});
			if (weak) {
				hideList();
			}
	...
	_list.controller->showItemRequests(
		...
		const auto &messages = _history->agramSecretChatId()
			? _localMessages
			: _apiSearch.messages().messages;
	...
	_bottomBar->buttonCalendarToggleOn(_topBar->fromValue(
		...
		return !_history->agramSecretChatId() && !from;
	}));
...
void ComposeSearch::Inner::searchLocal(const QString &query) {
	_localQuery = query;
	_localMessages.clear();
	const auto needle = query.trimmed();
	if (!needle.isEmpty()) {
		const auto &items = _history->clientSideMessages();
		for (auto i = items.rbegin(); i != items.rend(); ++i) {
			const auto item = *i;
			if (item->originalText().text.contains(
					needle,
					Qt::CaseInsensitive)) {
				_localMessages.push_back(item->fullId());
			}
		}
	}
	_list.controller->setQuery(query);
	_bottomBar->setTotal(int(_localMessages.size()));
	_list.controller->addItems(_localMessages, true);
}
