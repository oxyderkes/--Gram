...
, _secretChats(std::make_unique<SecretChats::Manager>(this))
...
void Session::startHeavyLocalData() {
	if (_heavyLocalDataStarted) {
		return;
	}
	_heavyLocalDataStarted = true;
	data().ensureAgramArchivesLoaded();
...
void Session::finishLogout() {
	unlockTerms();
	_secretChats->prepareForSessionClear();
	data().prepareAgramLogout();
	data().clear();
