...
void Instance::applyValue(const QByteArray &key, const QByteArray &value) {
	...
	ParseKeyValue(key, value, [&](ushort keyIndex, QString &&value) {
		const auto owner = _derived ? _derived : this;
		if (keyIndex >= _nonDefaultSet.size()
			|| keyIndex >= owner->_nonDefaultSet.size()
			|| keyIndex >= owner->_values.size()) {
			LOG(("Lang Error: Key index is out of range: %1 (%2)"
				).arg(keyIndex).arg(QString::fromLatin1(key)));
			return;
		}
		_nonDefaultSet[keyIndex] = 1;
		if (!_derived) {
			_values[keyIndex] = std::move(value);
		} else if (!_derived->_nonDefaultSet[keyIndex]) {
			_derived->_values[keyIndex] = std::move(value);
		}
