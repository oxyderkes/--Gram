...
void SessionController::openPeerStory(
	...
	if (!peer->isSelf()) {
		ConfirmGhostModeAction(
			this,
			u"История останется отмеченной как непросмотренная. Реакции, ответы и другие действия могут раскрыть вашу активность."_q,
			u"Открыть"_q,
			[=, context = std::move(context)]() mutable {
				openPeerStoryConfirmed(peer, storyId, std::move(context));
			});
		return;
	}
	openPeerStoryConfirmed(peer, storyId, std::move(context));
...
void SessionController::openPeerStoryConfirmed(
		not_null<PeerData*> peer,
		StoryId storyId,
		Data::StoriesContext context) {
	using namespace Media::View;
	...
	if (from) {
		...
		const auto done = crl::guard(&_storyOpenGuard, [=] {
			openPeerStoryConfirmed(peer, storyId, context);
		});
