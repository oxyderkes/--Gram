...
void Filler::addInfo() {
	...
	const auto secretChat = (_request.key.secretChat() != nullptr);
	...
	_addAction(text, [=] {
		if (const auto strong = weak.get()) {
			if (base::IsCtrlPressed()) {
				...
				controller->showPeerInfo(infoPeer);
			} else {
...
void Filler::addSecretChat() {
	...
	const auto controller = _controller;
	_addAction(
		u"Начать секретный чат"_q,
		[=] { SecretChats::Start(controller, user); },
		&st::menuIconLock);
}
...
void Filler::fill() {
	if (_request.key.secretChat()) {
		fillSecretChatActions();
	} else if (_folder) {
		fillArchiveActions();
	} else if (_sublist && _peer->isSelf()) {
...
void Filler::fillSecretChatActions() {
	const auto secret = _request.key.secretChat();
	if (!secret) {
		return;
	}
	const auto controller = _controller;
	const auto chatId = secret->chatId();
	addInfo();
	_addAction(
		u"Таймер самоуничтожения"_q,
		[=] { SecretChats::ShowTtlSettings(controller, chatId); },
		&st::menuIconLock);
	_addAction(
		u"Параметры секретного чата"_q,
		[=] { SecretChats::ShowSettings(controller, chatId); },
		&st::menuIconInfo);
	_addAction(PeerMenuCallback::Args{ .isSeparator = true });
	_addAction({
		.text = u"Завершить секретный чат"_q,
		.handler = [=] {
			controller->show(Ui::MakeConfirmBox({
				.text = u"Завершить секретный чат? Отправлять новые сообщения после этого будет нельзя."_q,
				.confirmed = [=] {
					controller->session().secretChats().discard(chatId, false);
				},
...
void FillSenderUserpicMenu(
	...
	if (searchInEntry) {
		...
		if (groupPeer) {
			addAction(tr::lng_agram_export_user_messages(tr::now), [=] {
				base::call_delayed(st::defaultPopupMenu.showDuration, [=] {
					Core::App().exportManager().startFromUser(
						groupPeer,
						peer);
				});
			}, &st::menuIconExport);
