...
void BuildAgramSection(SectionBuilder &builder) {
	const auto session = builder.session();

	builder.addSkip();
	builder.addSubsectionTitle({
		.id = u"agram/section"_q,
		.title = tr::lng_agram_settings_title(),
		.keywords = {
			u"agram"_q,
			u"deleted"_q,
			u"messages"_q,
	...
	const auto toggle = builder.addButton({
		.id = u"agram/save_deleted"_q,
		.title = tr::lng_agram_save_deleted(),
		.st = &st::settingsButtonNoIcon,
		.toggled = rpl::single(
			session->local().readPref<bool>(kAgramSaveDeletedPref)),
		.keywords = {
			u"deleted"_q,
			u"messages"_q,
			u"archive"_q,
	...
	if (toggle) {
		toggle->toggledChanges(
			...
			session->local().writePref<bool>(
				kAgramSaveDeletedPref,
				enabled);
			if (enabled) {
				session->data().ensureAgramArchivesLoaded();
			}
		}, toggle->lifetime());
	}

	builder.addSkip();
	builder.addDividerText(tr::lng_agram_save_deleted_about());

	const auto prefetchEnabled = session->local().readPref<bool>(
		kAgramPrefetchMediaPref,
		session->local().readPref<bool>(kAgramSaveDeletedPref));
	const auto prefetchToggle = builder.addButton({
		.id = u"agram/prefetch_media"_q,
		.title = tr::lng_agram_prefetch_media(),
		.st = &st::settingsButtonNoIcon,
		.toggled = rpl::single(prefetchEnabled),
		.keywords = {
			u"media"_q,
			u"download"_q,
			u"archive"_q,
	...
	if (prefetchToggle) {
		prefetchToggle->toggledChanges(
		) | rpl::on_next([=](bool enabled) {
			session->local().writePref<bool>(
				kAgramPrefetchMediaPref,
				enabled);
		}, prefetchToggle->lifetime());
	}

	builder.addSkip();
	builder.addDividerText(tr::lng_agram_prefetch_media_about());

	const auto editHistoryEnabled = session->local().readPref<bool>(
		kAgramSaveEditHistoryPref,
		session->local().readPref<bool>(kAgramSaveDeletedPref));
	const auto editToggle = builder.addButton({
		.id = u"agram/save_edit_history"_q,
		.title = tr::lng_agram_save_edit_history(),
		.st = &st::settingsButtonNoIcon,
		.toggled = rpl::single(editHistoryEnabled),
		.keywords = {
			u"edit"_q,
			u"history"_q,
			u"versions"_q,
	...
	builder.addSkip();
	builder.addDividerText(tr::lng_agram_save_edit_history_about());
}
