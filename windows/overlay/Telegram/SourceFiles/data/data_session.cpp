...
constexpr auto kAgramSaveDeletedPref = "agram-save-deleted-messages";
constexpr auto kAgramSaveEditHistoryPref = "agram-save-edit-history";
constexpr auto kAgramPrefetchMediaPref = "agram-prefetch-archive-media";
constexpr auto kAgramDeletedArchivePref = "agram-deleted-message-archive-v1";
constexpr auto kAgramEditHistoryPref = "agram-edit-history-v1";
constexpr auto kAgramDeletedArchiveVersion = quint32(1);
constexpr auto kAgramEditHistoryVersion = quint32(1);
constexpr auto kAgramDeletedArchiveLimit = quint32(20000);
constexpr auto kAgramDeletedArchiveReadLimit = quint32(100000);
constexpr auto kAgramEditHistoryLimit = quint32(10000);
constexpr auto kAgramEditHistoryReadLimit = quint32(20000);
constexpr auto kAgramEditHistoryPerMessageLimit = size_t(20);
constexpr auto kAgramDeletedMessageSizeLimit = quint32(1024 * 1024);
constexpr auto kAgramSnapshotCountLimit = size_t(5000);
constexpr auto kAgramSnapshotBytesLimit = size_t(32 * 1024 * 1024);
constexpr auto kAgramRestoredDeletedPerChatLimit = size_t(2000);
constexpr auto kAgramArchiveWriteDelay = crl::time(5000);

using ViewElement = HistoryView::Element;

[[nodiscard]] QByteArray SerializeAgramMessage(const MTPMessage &message) {
...
[[nodiscard]] std::optional<MTPMessage> DeserializeAgramMessage(
...
[[nodiscard]] bool AgramEditHistoryEnabled(Storage::Account &local) {
	return local.readPref<bool>(
		kAgramSaveEditHistoryPref,
		local.readPref<bool>(kAgramSaveDeletedPref));
}

[[nodiscard]] bool AgramPrefetchMediaEnabled(Storage::Account &local) {
	return local.readPref<bool>(
		kAgramPrefetchMediaPref,
		local.readPref<bool>(kAgramSaveDeletedPref));
}

[[nodiscard]] QString AgramMessageText(const MTPMessage &message) {
...
[[nodiscard]] TimeId AgramEditDate(const MTPMessage &message) {
	return message.match([](const MTPDmessageEmpty &) {
		return TimeId();
	}, [](const MTPDmessageService &) {
		return TimeId();
...
, _agramArchiveWriteTimer([=] { writeAgramArchives(); })
...
void Session::prepareAgramLogout() {
	_agramArchiveWriteTimer.cancel();
	_agramMessageSnapshots.clear();
	_agramMessageSnapshotsBytes = 0;
	_agramDeletedMessages.clear();
	_agramEditHistory.clear();
	_agramEditVersionsCount = 0;
	_agramDeletedMessagesDirty = false;
	_agramEditHistoryDirty = false;
	_agramDeletedMessagesLoaded = true;
	_agramEditHistoryLoaded = true;
}
...
Session::~Session() {
	if (_agramArchiveWriteTimer.isActive()) {
		writeAgramArchives();
	}
}
...
void Session::updateEditedMessage(const MTPMessage &data) {
	...
	rememberAgramEditedMessage(data, existing);
	rememberAgramMessage(data);
...
void Session::processMessagesDeleted(
	...
	for (const auto &messageId : data) {
		...
		if (list && i != list->end()) {
			if (preserveAgramDeletedMessage(i->second)) {
				continue;
			}
...
void Session::processNonChannelMessagesDeleted(const QVector<MTPint> &data) {
	...
	for (const auto &messageId : data) {
		if (const auto item = nonChannelMessage(messageId.v)) {
			if (preserveAgramDeletedMessage(item)) {
				continue;
			}
			const auto history = item->history();
...
void Session::rememberAgramMessage(const MTPMessage &data) {
	if (!_session->local().readPref<bool>(kAgramSaveDeletedPref)
		&& !AgramEditHistoryEnabled(_session->local())) {
		return;
	}
	const auto peerId = PeerFromMessage(data);
	const auto messageId = IdFromMessage(data);
	if (!peerId || !IsServerMsgId(messageId)
		|| data.type() == mtpc_messageEmpty) {
		return;
	}
	const auto raw = SerializeAgramMessage(data);
	if (raw.isEmpty() || raw.size() > kAgramDeletedMessageSizeLimit) {
		return;
	}
	const auto id = FullMsgId{ peerId, messageId };
	if (const auto i = _agramMessageSnapshots.find(id);
			i != end(_agramMessageSnapshots)) {
		_agramMessageSnapshotsBytes -= size_t(i->second.size());
		_agramMessageSnapshots.erase(i);
	}
	const auto bytes = size_t(raw.size());
	while (!_agramMessageSnapshots.empty()
		&& (_agramMessageSnapshots.size() >= kAgramSnapshotCountLimit
			|| _agramMessageSnapshotsBytes + bytes
				> kAgramSnapshotBytesLimit)) {
		const auto first = _agramMessageSnapshots.begin();
		_agramMessageSnapshotsBytes -= size_t(first->second.size());
		_agramMessageSnapshots.erase(first);
	}
	if (bytes > kAgramSnapshotBytesLimit) {
		return;
	}
	_agramMessageSnapshots.emplace(id, raw);
	_agramMessageSnapshotsBytes += bytes;
}

void Session::rememberAgramEditedMessage(
		const MTPMessage &data,
		not_null<HistoryItem*> item) {
	if (!AgramEditHistoryEnabled(_session->local())
		|| item->ttlDestroyAt()
		|| (item->media() && item->media()->ttlSeconds())) {
		return;
	}
	ensureAgramArchivesLoaded();
	const auto id = item->fullId();
	const auto previous = _agramMessageSnapshots.find(id);
	if (previous == end(_agramMessageSnapshots)) {
		return;
	}
	const auto now = SerializeAgramMessage(data);
	if (now == previous->second) {
		return;
	}
	const auto changedAt = AgramEditDate(data);
	if (!changedAt) {
		return;
	}
	const auto &raw = previous->second;
	auto &versions = _agramEditHistory[id];
	if (!versions.empty() && versions.back().message == raw) {
		return;
	}
	versions.push_back({
		.changedAt = changedAt,
	...
	++_agramEditVersionsCount;
	if (versions.size() > kAgramEditHistoryPerMessageLimit) {
		versions.erase(versions.begin());
		--_agramEditVersionsCount;
	}
	while (_agramEditVersionsCount > kAgramEditHistoryLimit
		&& !_agramEditHistory.empty()) {
		auto first = _agramEditHistory.begin();
		first->second.erase(first->second.begin());
		--_agramEditVersionsCount;
		if (first->second.empty()) {
			_agramEditHistory.erase(first);
		}
	}
	_agramEditHistoryDirty = true;
	scheduleWriteAgramArchives();
}

void Session::prefetchAgramMedia(HistoryItem *item) {
	if (!item
		|| item->out()
		|| !AgramPrefetchMediaEnabled(_session->local())
		|| item->ttlDestroyAt()) {
		return;
	}
	...
	if (const auto photo = media->photo()) {
		photo->createMediaView()->automaticLoad(item->fullId(), item);
	} else if (const auto document = media->document()) {
		document->createMediaView()->automaticLoad(item->fullId(), item);
	}
}

bool Session::preserveAgramDeletedMessage(
		not_null<HistoryItem*> item) {
	if (!_session->local().readPref<bool>(kAgramSaveDeletedPref)
		|| item->ttlDestroyAt()
		|| (item->media() && item->media()->ttlSeconds())) {
	...
	const auto id = FullMsgId{ item->history()->peer->id, item->id };
	const auto snapshot = _agramMessageSnapshots.find(id);
	const auto archived = _agramDeletedMessages.find(id);
	if (snapshot == end(_agramMessageSnapshots)
		&& archived == end(_agramDeletedMessages)) {
		return false;
	}
	if (archived == end(_agramDeletedMessages)
		&& _agramDeletedMessages.size() >= kAgramDeletedArchiveLimit) {
		return false;
	}
	item->markAgramDeletedOnServer();
	if (snapshot != end(_agramMessageSnapshots)
		&& (archived == end(_agramDeletedMessages)
			|| archived->second != snapshot->second)) {
		_agramDeletedMessages[id] = snapshot->second;
		_agramDeletedMessagesDirty = true;
		scheduleWriteAgramArchives();
	}
	return true;
}

void Session::forgetAgramDeletedMessage(not_null<HistoryItem*> item) {
	const auto id = FullMsgId{ item->history()->peer->id, item->id };
	if (_agramDeletedMessages.remove(id)) {
		_agramDeletedMessagesDirty = true;
	}
	const auto i = _agramEditHistory.find(id);
	if (i != end(_agramEditHistory)) {
		_agramEditVersionsCount -= i->second.size();
		_agramEditHistory.erase(i);
		_agramEditHistoryDirty = true;
	}
	if (_agramDeletedMessagesDirty || _agramEditHistoryDirty) {
		scheduleWriteAgramArchives();
	}
}

void Session::forgetAgramDeletedMessages(PeerId peerId) {
	auto changed = false;
	for (auto i = begin(_agramDeletedMessages);
			i != end(_agramDeletedMessages);) {
		if (i->first.peer == peerId) {
			i = _agramDeletedMessages.erase(i);
			changed = true;
		} else {
			++i;
		}
	...
	for (auto i = begin(_agramEditHistory);
			i != end(_agramEditHistory);) {
		if (i->first.peer == peerId) {
			_agramEditVersionsCount -= i->second.size();
			i = _agramEditHistory.erase(i);
			_agramEditHistoryDirty = true;
		} else {
			++i;
		}
...
void Session::ensureAgramArchivesLoaded() {
	if (!_agramDeletedMessagesLoaded
		&& _session->local().readPref<bool>(kAgramSaveDeletedPref)) {
		loadAgramDeletedMessages();
	}
	if (!_agramEditHistoryLoaded
		&& AgramEditHistoryEnabled(_session->local())) {
		loadAgramEditHistory();
	}
}

bool Session::hasAgramEditHistory(FullMsgId itemId) const {
	const auto i = _agramEditHistory.find(itemId);
	return i != end(_agramEditHistory) && !i->second.empty();
}

std::vector<AgramEditVersion> Session::agramEditHistory(
		FullMsgId itemId) const {
	auto result = std::vector<AgramEditVersion>();
	const auto i = _agramEditHistory.find(itemId);
	if (i == end(_agramEditHistory)) {
		return result;
	}
	result.reserve(i->second.size());
	for (const auto &version : i->second) {
		if (const auto parsed = DeserializeAgramMessage(version.message)) {
			result.push_back({
				.changedAt = version.changedAt,
				.text = AgramMessageText(*parsed),
			});
		}
	}
	return result;
...
void Session::loadAgramDeletedMessages() {
	if (_agramDeletedMessagesLoaded
		|| !_session->local().readPref<bool>(kAgramSaveDeletedPref)) {
		return;
	}
	_agramDeletedMessagesLoaded = true;
	auto serialized = _session->local().readPref<QByteArray>(
		kAgramDeletedArchivePref);
	if (serialized.isEmpty()) {
		return;
	}
	auto stream = QDataStream(&serialized, QIODevice::ReadOnly);
	stream.setVersion(QDataStream::Qt_5_1);
	auto version = quint32();
	auto count = quint32();
	stream >> version >> count;
	if (stream.status() != QDataStream::Ok
		|| version != kAgramDeletedArchiveVersion
		|| count > kAgramDeletedArchiveReadLimit) {
		return;
	}
	...
	for (auto index = quint32(); index != count; ++index) {
		...
		if (stream.status() != QDataStream::Ok
			|| size == 0
			|| size > kAgramDeletedMessageSizeLimit
			|| size > quint32(stream.device()->bytesAvailable())) {
			return;
		}
		auto raw = QByteArray(size, Qt::Uninitialized);
		if (stream.readRawData(raw.data(), size) != int(size)) {
			return;
		}
		const auto parsed = DeserializeAgramMessage(raw);
		if (!parsed) {
			continue;
		}
		const auto peerId = PeerFromMessage(*parsed);
		const auto messageId = IdFromMessage(*parsed);
		if (!peerId || !IsServerMsgId(messageId)) {
			continue;
		}
		const auto id = FullMsgId{ peerId, messageId };
		if (index + kAgramDeletedArchiveLimit >= count) {
			_agramDeletedMessages[id] = raw;
...
void Session::restoreAgramDeletedMessages(PeerId peerId) {
	if (!_session->local().readPref<bool>(kAgramSaveDeletedPref)) {
		return;
	}
	ensureAgramArchivesLoaded();
	auto pending = std::vector<QByteArray>();
	pending.reserve(kAgramRestoredDeletedPerChatLimit);
	for (auto i = _agramDeletedMessages.rbegin();
			i != _agramDeletedMessages.rend(); ++i) {
		if (i->first.peer != peerId || message(i->first)) {
			continue;
		}
		pending.push_back(i->second);
		if (pending.size() >= kAgramRestoredDeletedPerChatLimit) {
			break;
		}
	}
	for (auto i = pending.rbegin(); i != pending.rend(); ++i) {
		if (const auto parsed = DeserializeAgramMessage(*i)) {
			if (const auto added = addNewMessage(
					*parsed,
					MessageFlags(),
					NewMessageType::Existing)) {
				added->markAgramDeletedOnServer();
			}
		}
	}
}

void Session::loadAgramEditHistory() {
	if (_agramEditHistoryLoaded
		|| !AgramEditHistoryEnabled(_session->local())) {
		return;
	}
	...
	stream >> version >> count;
	if (stream.status() != QDataStream::Ok
		|| version != kAgramEditHistoryVersion
		|| count > kAgramEditHistoryReadLimit) {
		return;
	}
	for (auto index = quint32(); index != count; ++index) {
		auto changedAt = qint64();
		auto size = quint32();
		stream >> changedAt >> size;
		if (stream.status() != QDataStream::Ok
			|| size == 0
			|| size > kAgramDeletedMessageSizeLimit
			|| size > quint32(stream.device()->bytesAvailable())) {
		...
		if (index + kAgramEditHistoryLimit < count) {
			continue;
		}
		auto &versions = _agramEditHistory[
			FullMsgId{ peerId, messageId }];
		if (versions.size() >= kAgramEditHistoryPerMessageLimit) {
			continue;
		}
		versions.push_back({
			.changedAt = TimeId(changedAt),
			.message = std::move(raw),
		});
		++_agramEditVersionsCount;
	}
}

void Session::scheduleWriteAgramArchives() {
	if (!_agramArchiveWriteTimer.isActive()) {
		_agramArchiveWriteTimer.callOnce(kAgramArchiveWriteDelay);
	}
}

void Session::writeAgramArchives() {
	_agramArchiveWriteTimer.cancel();
	if (base::take(_agramDeletedMessagesDirty)) {
		writeAgramDeletedMessages();
	}
	if (base::take(_agramEditHistoryDirty)) {
		writeAgramEditHistory();
	}
}

void Session::writeAgramDeletedMessages() {
	if (_agramDeletedMessages.empty()) {
		_session->local().clearPref(kAgramDeletedArchivePref);
		return;
	}
	auto serialized = QByteArray();
	auto stream = QDataStream(&serialized, QIODevice::WriteOnly);
	stream.setVersion(QDataStream::Qt_5_1);
	stream << kAgramDeletedArchiveVersion;
	stream << quint32(_agramDeletedMessages.size());
	for (const auto &[id, raw] : _agramDeletedMessages) {
		Q_UNUSED(id);
		stream << quint32(raw.size());
...
void Session::writeAgramEditHistory() {
	if (_agramEditHistory.empty()) {
		_session->local().clearPref(kAgramEditHistoryPref);
		return;
	}
	auto serialized = QByteArray();
	auto stream = QDataStream(&serialized, QIODevice::WriteOnly);
	stream.setVersion(QDataStream::Qt_5_1);
	stream << kAgramEditHistoryVersion;
	stream << quint32(_agramEditVersionsCount);
	for (const auto &[id, versions] : _agramEditHistory) {
		Q_UNUSED(id);
		for (const auto &version : versions) {
			stream << qint64(version.changedAt);
			stream << quint32(version.message.size());
			stream.writeRawData(
				version.message.constData(),
				version.message.size());
		}
	}
	_session->local().writePref<QByteArray>(
		kAgramEditHistoryPref,
...
void Session::unregisterMessage(not_null<HistoryItem*> item) {
	const auto peerId = item->history()->peer->id;
	const auto itemId = item->id;
	const auto snapshot = _agramMessageSnapshots.find(
		FullMsgId{ peerId, itemId });
	if (snapshot != end(_agramMessageSnapshots)) {
		_agramMessageSnapshotsBytes -= size_t(snapshot->second.size());
		_agramMessageSnapshots.erase(snapshot);
	}
