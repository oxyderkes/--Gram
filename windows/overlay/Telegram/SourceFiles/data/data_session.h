...
public:
	...
	void forgetAgramDeletedMessage(not_null<HistoryItem*> item);
	void forgetAgramDeletedMessages(PeerId peerId);
	void ensureAgramArchivesLoaded();
	void restoreAgramDeletedMessages(PeerId peerId);
	[[nodiscard]] bool hasAgramEditHistory(FullMsgId itemId) const;
	[[nodiscard]] std::vector<AgramEditVersion> agramEditHistory(
		FullMsgId itemId) const;
...
private:
	...
	void rememberAgramMessage(const MTPMessage &data);
	void rememberAgramEditedMessage(
		const MTPMessage &data,
		not_null<HistoryItem*> item);
	void prefetchAgramMedia(HistoryItem *item);
	[[nodiscard]] bool preserveAgramDeletedMessage(
		not_null<HistoryItem*> item);
	void loadAgramDeletedMessages();
	void loadAgramEditHistory();
	void scheduleWriteAgramArchives();
	void writeAgramArchives();
	void writeAgramDeletedMessages();
	void writeAgramEditHistory();
	...
	base::flat_map<FullMsgId, QByteArray> _agramMessageSnapshots;
	size_t _agramMessageSnapshotsBytes = 0;
	base::flat_map<FullMsgId, QByteArray> _agramDeletedMessages;
	base::flat_map<
		FullMsgId,
		std::vector<AgramStoredEditVersion>> _agramEditHistory;
	size_t _agramEditVersionsCount = 0;
	base::Timer _agramArchiveWriteTimer;
	bool _agramDeletedMessagesLoaded = false;
	bool _agramEditHistoryLoaded = false;
	bool _agramDeletedMessagesDirty = false;
	bool _agramEditHistoryDirty = false;
