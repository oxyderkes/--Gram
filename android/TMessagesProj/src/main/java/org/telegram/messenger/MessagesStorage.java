...
//                }
    ...
    public ArrayList<Integer> markMessagesDeletedOnServer(long dialogId, ArrayList<Integer> messages) {
        ...
        SQLiteCursor cursor = null;
        SQLitePreparedStatement messagesState = null;
        SQLitePreparedStatement topicsState = null;
        try {
            String ids = TextUtils.join(",", messages);
            if (dialogId == 0) {
                cursor = database.queryFinalized(String.format(Locale.US, "SELECT uid, mid, data, custom_params, ttl FROM messages_v2 WHERE mid IN(%s) AND is_channel = 0", ids));
            } else {
                cursor = database.queryFinalized(String.format(Locale.US, "SELECT uid, mid, data, custom_params, ttl FROM messages_v2 WHERE mid IN(%s) AND uid = %d", ids, dialogId));
            }
            messagesState = database.executeFast("UPDATE messages_v2 SET custom_params = ? WHERE mid = ? AND uid = ?");
            topicsState = database.executeFast("UPDATE messages_topics SET custom_params = ? WHERE mid = ? AND uid = ?");
            database.beginTransaction();
            while (cursor.next()) {
                long messageDialogId = cursor.longValue(0);
                ...
                NativeByteBuffer oldCustomParams = cursor.byteBufferValue(3);
