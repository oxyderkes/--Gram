...
public class ApplicationLoader extends Application {
    ...
    private static final Object accountInitLock = new Object();
    private static final boolean[] accountConnectionsStarted = new boolean[UserConfig.MAX_ACCOUNT_COUNT];
    private static final boolean[] accountControllersInitialized = new boolean[UserConfig.MAX_ACCOUNT_COUNT];
    private static volatile boolean accountServicesReady;
    ...
    public static boolean isAccountConnectionStarted(int account) {
        synchronized (accountInitLock) {
            return account >= 0 && account < accountConnectionsStarted.length && accountConnectionsStarted[account];
        }
    }

    public static boolean isAccountInitialized(int account) {
        synchronized (accountInitLock) {
            return account >= 0 && account < accountControllersInitialized.length && accountControllersInitialized[account];
        }
    }

    public static void ensureAccountConnectionStarted(int account) {
        if (account < 0 || account >= UserConfig.MAX_ACCOUNT_COUNT) {
            return;
        }
        synchronized (accountInitLock) {
            if (accountConnectionsStarted[account]) {
                return;
            }
            accountConnectionsStarted[account] = true;
        }
        try {
            ConnectionsManager.getInstance(account);
        } catch (Throwable e) {
            synchronized (accountInitLock) {
                accountConnectionsStarted[account] = false;
            }
            FileLog.e(e);
        }
    }
    ...
    /**
     * Initializes the heavy per-account graph only when an account becomes
     * visible. Background accounts start with a lightweight connection and are
     * promoted here on selection or when their controller is otherwise needed.
     */
    public static void ensureAccountInitialized(int account) {
        if (account < 0 || account >= UserConfig.MAX_ACCOUNT_COUNT) {
            return;
        }
        UserConfig config = UserConfig.getInstance(account);
        config.loadConfig();
        ensureAccountConnectionStarted(account);
        synchronized (accountInitLock) {
            if (accountControllersInitialized[account]) {
                return;
            }
            accountControllersInitialized[account] = true;
        }
        try {
            ...
            if (accountServicesReady) {
                ContactsController.getInstance(account).checkAppAccount();
                DownloadController.getInstance(account);
            }
        } catch (Throwable e) {
            synchronized (accountInitLock) {
                accountControllersInitialized[account] = false;
            }
            FileLog.e(e);
        }
    }

    private static void scheduleBackgroundAccountConnections() {
        ...
        Collections.sort(accounts, (left, right) -> Integer.compare(
                UserConfig.getInstance(right).loginTime,
                UserConfig.getInstance(left).loginTime
        ));
        for (int i = 0; i < accounts.size(); i++) {
            final int account = accounts.get(i);
        ...
        try {
            ...
            BroadcastReceiver networkStateReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    for (int a = 0; a < UserConfig.MAX_ACCOUNT_COUNT; a++) {
                        if (isAccountConnectionStarted(a)) {
                            ConnectionsManager.getInstance(a).checkConnection();
                        }
                        if (isAccountInitialized(a)) {
