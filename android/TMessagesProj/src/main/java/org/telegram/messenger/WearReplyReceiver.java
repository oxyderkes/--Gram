...
public class WearReplyReceiver extends BroadcastReceiver {
    ...
    private void sendMessage(AccountInstance accountInstance, CharSequence text, long dialog_id, long topicId, int max_id, int[] voiceMsgIds) {
        ...
        if (topicId == 0) {
            ...
            int account = accountInstance.getCurrentAccount();
            if (!ghostMode.shouldSuppressReadReceipt(account)
                    || ghostMode.isReadOnInteractionEnabled(account)) {
                accountInstance.getMessagesController().markDialogAsReadExplicit(dialog_id, max_id, max_id, 0, false, topicId, 0, true, 0);
            }
        }
    }
}
