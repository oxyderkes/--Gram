    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chatActivityEnterView.setDelegate(new ChatActivityEnterView.ChatActivityEnterViewDelegate() {
            ...
            public void onMessageSend(CharSequence message, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars) {
                ...
                AgramContainerManager ghostMode = AgramContainerManager.getInstance();
                if (!ghostMode.shouldSuppressReadReceipt(currentMessageObject.currentAccount)
                        || ghostMode.isReadOnInteractionEnabled(currentMessageObject.currentAccount)) {
                    MessagesController.getInstance(currentMessageObject.currentAccount).markDialogAsReadExplicit(currentMessageObject.getDialogId(), currentMessageObject.getId(), Math.max(0, currentMessageObject.getId()), currentMessageObject.messageOwner.date, true, 0, 0, true, 0);
                }
