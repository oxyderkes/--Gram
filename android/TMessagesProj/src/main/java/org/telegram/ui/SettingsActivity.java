...
//        fragmentView.addView(navigationBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 0, Gravity.BOTTOM | Gravity.FILL_HORIZONTAL));
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        for (int a = 0; a < UserConfig.MAX_ACCOUNT_COUNT; a++) {
            if (UserConfig.getInstance(a).hasAccountEntry() && currentAccount != a) {
    ...
    public static class AccountCell extends LinearLayout implements Theme.Colorable {
        ...
        public void set(int account) {
            final UserConfig accountConfig = UserConfig.getInstance(account);
            final TLRPC.User user = accountConfig.getDisplayUser();

            avatarDrawable.setInfo(account, user);
            avatarView.getImageReceiver().setCurrentAccount(account);
            final String localAvatarPath = accountConfig.getLocalAccountAvatarPath();
            if (accountConfig.getAccountState() == UserConfig.ACCOUNT_STATE_BLOCKED && localAvatarPath != null) {
                avatarView.setImage(localAvatarPath, "50_50", avatarDrawable);
            } else {
                avatarView.setForUserOrChat(user, avatarDrawable);
            }
            final String name = user != null ? UserObject.getUserName(user) : getString(R.string.HiddenName);
            final String status = accountConfig.getAccountStatusText();
            final SpannableStringBuilder title = new SpannableStringBuilder(name);
            if (!TextUtils.isEmpty(status)) {
                int start = title.length();
                title.append(" · ").append(status);
                title.setSpan(new RelativeSizeSpan(0.82f), start, title.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                title.setSpan(new ForegroundColorSpan(Theme.getColor(
                        accountConfig.getAccountState() == UserConfig.ACCOUNT_STATE_BLOCKED
            ...
            int counter = accountConfig.isClientActivated()
                    ? MessagesStorage.getInstance(account).getMainUnreadCount()
                    : 0;
