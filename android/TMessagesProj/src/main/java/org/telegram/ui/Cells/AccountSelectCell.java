...
public class AccountSelectCell extends FrameLayout {
    ...
    public void setAccount(int account, boolean check) {
        ...
        UserConfig accountConfig = UserConfig.getInstance(accountNumber);
        TLRPC.User user = accountConfig.getDisplayUser();
        avatarDrawable.setInfo(account, user);
        String name = user != null
                ? ContactsController.formatName(user.first_name, user.last_name)
                : LocaleController.getString(R.string.HiddenName);
        String status = accountConfig.getAccountStatusText();
        ...
        String localAvatarPath = accountConfig.getLocalAccountAvatarPath();
        if (accountConfig.getAccountState() == UserConfig.ACCOUNT_STATE_BLOCKED && localAvatarPath != null) {
            imageView.setImage(localAvatarPath, "50_50", avatarDrawable);
        } else {
            imageView.setForUserOrChat(user, avatarDrawable);
        }
